#ifndef dafne_ObjectSystematicProducer_h
#define dafne_ObjectSystematicProducer_h
...
namespace flashgg {
    ...
    class ObjectSystematicProducer_dafne : public edm::EDProducer
    {
    public:

        ObjectSystematicProducer_dafne( const edm::ParameterSet & );
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    ObjectSystematicProducer_dafne<flashgg_object, param_var, output_container>::ObjectSystematicProducer_dafne( const ParameterSet &iConfig ) :
        globalVars_(iConfig),
        ObjectToken_( consumes<View<flashgg_object> >( iConfig.getParameter<InputTag>( "src" ) ) )
    {
        //        edm::Service<edm::RandomNumberGenerator> rng;
        //        if( ! rng.isAvailable() ) {
        //            throw cms::Exception( "Configuration" ) << "ObjectSystematicProducer_dafne requires the RandomNumberGeneratorService  - please add to configuration";
        //        }
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    void ObjectSystematicProducer_dafne<flashgg_object, param_var, output_container>::ApplyCorrections( flashgg_object &y,
            shared_ptr<BaseSystMethod<flashgg_object, param_var> > CorrToShift,
            param_var syst_shift )
    {
        float theWeight = 1.;
        //        std::cout << " 1d before 1d " << std::endl;
        //        std::cout << " In ObjectSystematicProducer_dafne::ApplyCorrections pt m " << y.pt() << " " << y.mass() << std::endl;
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    void ObjectSystematicProducer_dafne<flashgg_object, param_var, output_container>::ApplyNonCentralWeights( flashgg_object &y )
    {
        for( unsigned int ncorr = 0; ncorr < Corrections_.size(); ncorr++ ) {
            if( Corrections_.at( ncorr )->makesWeight() ) {
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    void ObjectSystematicProducer_dafne<flashgg_object, param_var, output_container>::produce( Event &evt, const EventSetup & setup )
    {
